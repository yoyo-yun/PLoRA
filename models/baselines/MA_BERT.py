...
class UserItemEmbeddings(nn.Module):
    ...
    def forward(self, user_ids, item_ids):
        ...
        return self.user_embeddings(user_ids), self.user_embeddings(user_ids)
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, cus_config):
        ...
        self.p_q = nn.Linear(cus_config.prd_dim+cus_config.usr_dim, self.all_head_size, bias=False)
        self.p_k = nn.Linear(cus_config.prd_dim+cus_config.usr_dim, self.all_head_size, bias=False)
        self.p_v = nn.Linear(cus_config.prd_dim+cus_config.usr_dim, self.all_head_size, bias=False)
    ...
    def forward(
        ...
        if user_states is not None and item_states is not None:
            ...
            i_u_states = torch.cat([item_states, user_states], -1)

            mixed_query_layer = mixed_query_layer + self.p_q(u_i_states)
            mixed_key_layer = mixed_key_layer + self.p_k(i_u_states)
            mixed_value_layer = mixed_value_layer + self.p_v(i_u_states)
...
class BertEncoder(nn.Module):
    def __init__(self, config, cus_config):
        ...
        self.more_layer = nn.ModuleList([BertLayer(config, cus_config) for _ in range(cus_config.more_layers)])
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.more_layer):
            if output_hidden_states:
                all_hidden_states = all_hidden_states + (hidden_states,)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        if user_ids is None:
            embedding_user = torch.zeros((embedding_output.shape[0], 1, self.user_item_embeddings.user_embeddings.weight.shape[1]),
                                                         device=embedding_output.device)
            embedding_item = torch.zeros_like(embedding_user, device=embedding_output.device)
        else:
            embedding_user, embedding_item = self.user_item_embeddings(user_ids, item_ids)
            embedding_item = embedding_item.zero_()
...
class MABertForSequenceClassification(BertPreTrainedModel):
